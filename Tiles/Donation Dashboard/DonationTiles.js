...
//Tile 4 defferedDonation .
function defferedDonations() {
        ...
        if (retrieveReq.readyState == 4 && retrieveReq.status == 200) {
            ...
            for (var don = 0; don < response.length; don++) {
                if (response[don].new_donationtype = 100000000) {
                    defferedDonationAmount += response[don].new_amount;
                }
            }
            var Differed = defferedDonationAmount;
...
function totalDonors(data) {
    ...
    retrieveReq.onreadystatechange = function () {
        if (retrieveReq.readyState == 4 && retrieveReq.status == 200) {
            ...
            document.getElementById("defferedDonation").innerHTML = (Math.round(parseFloat(data.DifferedPercentage) * 100) / 100).toLocaleString() + "%";
